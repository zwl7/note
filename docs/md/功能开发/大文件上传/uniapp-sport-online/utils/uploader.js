...
class Uploader {
	...
	async upload() {
		...
		try {
			...
			if (this.config.testChunks) {
				...
				this.identifier = this.identifier.slice(0, this.identifier.length - 1) + "X";
			} else {
		...
		if (this.config.testChunks && this.config.verifyUrl) {
			...
			if (!needUpload) {
				...
				this.chunksIndexNeedRead = this.chunksIndexNeedRead.filter(
					(v) => !uploadedChunks.includes(v)
				);
				this.chunksIndexNeedSend = this.chunksIndexNeedSend.filter(
					(v) => !uploadedChunks.includes(v)
				);
				this.uploadedChunks = uploadedChunks.sort();
			}
		}
		console.log(this.chunksIndexNeedRead)
		console.log(this.chunksIndexNeedSend)
	...
	_upload() {
		...
		console.log(this.chunksQueue.length);
	...
	readFileChunk() {
		...
		const chunks = Math.min(
			chunksIndexNeedRead.length,
			maxLoadChunks - chunksQueue.length
		);
		// 异步读取
		logger.debug(
			`readFileChunk chunks: ${chunks}, chunksIndexNeedRead`,
	...
	uploadChunk() {
		...
		// 达到最大并发度
		if (Object.keys(this.uploadTasks).length === this.config.maxConcurrency)
			return;
		...
		formData.append("file_hash", identifier);
		formData.append("chunk", index + 1);
		formData.append("total_chunk", this.totalChunks);
		formData.append("file_hash", identifier);
		formData.append("belong_file_hash", identifier);
		formData.appendFileBuffer('file', identifier, chunk)
		let form_data = formData.getData();
		this._requestAsync({
					url: uploadUrl,
					data: form_data.buffer,
					header: {
						...header,
						'content-type': form_data.contentType
					},
					method: "POST",
				},
				(task) => {
			...
			.then((res) => {
				...
				// 所有分片发送完毕
				if (this.chunksSend === this.chunksNeedSend) {
					console.log('所有分片发送完毕', res.data)
					this.emit("uploadDone", res?.data?.data.url);
				}
				return null;
			})
			.catch((res) => {
	...
	async computeMD5() {
		...
		if (sliceNum < 10) {
			slice_index = 2
		} else if (sliceNum >= 10 && sliceNum < 30) {
			slice_index = 3
		} else if (sliceNum >= 30) {
			slice_index = 5
		}
		for (let i = 0; i < sliceNum; i++) {
			...
			const [readFileErr, readFileResp] = await awaitWrap(
				readFileAsync({
					filePath: tempFilePath,
					position,
					length,
	...
	async verifyRequest() {
		...
		let res = await this._requestAsync({
			url: verifyUrl,
			method: "POST",
			header: {
			...
			data: {
				file_hash: this.identifier,
				suffix: 'mp4',
				total_chunk: this.totalChunks,
			},
		});
		res = res.data
		let verifyResp = {
			needUpload: res.data.is_upload == 2 ? true : false,
			uploadedChunks: [],
			url: res.data.url
		}
		if (res.data.chunks) {
			verifyResp.uploadedChunks = res.data.chunks.split(',').map((e) => Number(e) - 1)
		}
		return verifyResp;
	}
}
