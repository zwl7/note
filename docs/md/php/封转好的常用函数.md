...
class AESCipher {
    ...
    private function __construct() {
        $salt = env('APP_KEY') ?: $this->slat;

        $this->key = substr(hash('sha256', $salt, true), 0, 32);

        $this->iv = substr(hash('sha256', $salt, true), 0, 16);
    }

    public function encrypt(string $data,$is_diff_iv = true): string {

        if (empty($data)) {
            return "";
        }

        $iv = $is_diff_iv ? openssl_random_pseudo_bytes(openssl_cipher_iv_length($this->algorithm)) : $this->iv;
        $encrypted = openssl_encrypt($data, $this->algorithm, $this->key, 0, $iv);
        if ($encrypted === false) {
            throw new PPosException('无效的参数.');
        }
