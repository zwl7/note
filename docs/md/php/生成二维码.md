...
class QrCodeServer
{
    ...
    protected $_file_name       = 'upload/qrcode';    // 写入文件路径
...
{
    $file_name = 'upload/qrcode';  // 定义保存目录
    $config['file_name'] = $file_name;
    $config['generate'] = 'writefile';
    $qr_code = new \app\common\QrCodeServer($config);
    $res = $qr_code->createServer($url);
    if ($res['success']) {
        return '/' . strtr($res['data']['url'],'\\','/');
    } else {
        return false;
    }
}
...
```php
	 $res = getQrCode('https://www.taobao.com/');
	 var_dump($res);die;
```

![image-20201010150119567](.\img\image-20201010150119567.png)