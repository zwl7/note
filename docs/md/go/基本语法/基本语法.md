...
`new` 和 `make` 在 Go 中分工明确：`new` 用于任意类型的零值指针分配，`make` 专用于切片、映射和通道的初始化。实际编码中，`new` 较少直接出现，而 `make` 是这三种类型创建时的标配。
...
![image-20250713102705192](../../../md/img/image-20250713102705192.png)
...
func main() {
	...
	//输出：{"name":"张三","age":18,"birthday":"2025-07-12T16:06:32.783726+08:00"}
	...
	//{"name":"张三","age":18,"birthday":"2025-07-12"}
...
func (e specialError) Error() string {
	return fmt.Sprintf("errCode: %d, errMsg: %s", e.errCode, e.msg)
}

type specialErrorTwo struct {
	msg      string
	errCode  int
	errScene string
}
...
// 实现error接口
func (e specialErrorTwo) Error() string {
	return fmt.Sprintf("errCode: %d, errMsg: %s, errScene: %s", e.errCode, e.msg, e.errScene)
}
...
func main() {
	...
	if ok {
		...
		//// 2.使用interface{} 断言 v是interface{} 类型 断言interface{}究竟是什么类型？
		switch v.(type) {
		case int:
			println("+++++++++++++")
			fmt.Println("v isint")
		...
		//
		println("+++++++++++++")
		fmt.Println("没有gender字段")
		println("+++++++++++++")
	}

	// 3.使用接口类型断言

	demo2 := errors.New("普通错误")

	v2, ok2 := demo2.(*specialError)
	if ok2 {
		fmt.Println("demo2 is specialError")
		...
		fmt.Println("demo2 is not specialError")
	}

	//把

	demo3 := specialError{msg: "i am error", errCode: 100}

	println("+++++++++++++")
	//可以成功，因为本来demo3 就是 specialError 类型
	fmt.Println(testError(demo3).(specialError).Error())

	//报错，因为demo3 不是 specialErrorTwo 类型 会报错：panic: interface conversion: error is main.specialError, not main.specialErrorTwo
	//fmt.Println(testError(demo3).(specialErrorTwo).Error())
	println("+++++++++++++")

	demo3Error := testError(demo3)
	//这种情况下最好断言一下
	switch demo3Error.(type) {
	case specialError:
		println("+++++++++++++")
		fmt.Println("demo3Error is specialError")
		println("+++++++++++++")
	case specialErrorTwo:
		println("+++++++++++++")
		fmt.Println("demo3Error is specialErrorTwo")
		println("+++++++++++++")
	default:
		println("+++++++++++++")
		fmt.Println("demo3Error is not specialError or specialErrorTwo")
		println("+++++++++++++")
	}
...
没有gender字段
+++++++++++++
demo2 is not specialError
+++++++++++++
+++++++++++++
errCode: 100, errMsg: i am error
+++++++++++++
errCode: 100, errMsg: i am error
+++++++++++++
+++++++++++++
